...
public class Interpreter implements Expr.Visitor<Object>, Stmt.Visitor<Void> {
  ...
  public Object visitLogicalExpr(Expr.Logical expr) {
    ...
    // A || B short circuit A true
    if (expr.operator.type == TokenType.OR) {
      if (isTruthy(left)) {
        return left;
      }
    } else {
      // A && B short circuit A false
      if (!isTruthy(left)) {
        return left;
      }
    }
  ...
  public Object visitSuperExpr(Expr.Super expr) {
    ...
    // resolve super instance by peeking a prior env
    LoxInstance superInstance = (LoxInstance) environment.getAt(distance - 1, "this");
  ...
  public Void visitClassStmt(Stmt.Class stmt) {
    ...
    for (Stmt.Function method : stmt.methods) {
      ...
      LoxFunction function = new LoxFunction(method, environment, methodName.equals("init"));
      methods.put(methodName, function);
    }

    LoxClass klass = new LoxClass(stmt.name.lexeme, (LoxClass) superclass, methods);
    if (superclass != null) {
      // pop back previous state
